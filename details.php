...
$insert=0;
 ...
 $sql="INSERT INTO `hospital`.`pdetail` (`uid`, `name`, `age`, `gender`, `street`, `city`, `state`, `pincode`, `history`, `date`, `time`, `doctor`,`email`) VALUES ( '$uid','$name', '$age', '$gender', '$street', '$city', '$state', '$pincode', '$history', '$date', '$time', '$doctor','$email');";
  ...
  {
  $pid_sql="SELECT * from `hospital`.`pdetail` where `email`='$email'";
  $pid_result=$con->query(($pid_sql));
  $pid = $pid_result->fetch_array()[1] ?? '';
  $nm=$pid_result->fetch_array()[2] ?? '';
  $doc = $pid_result->fetch_array()[12] ?? '';
    ...
    <div class="form-signup">
      ...
      <form action="details.php" method="post">
        <div class="mb-3">
          ...
          <input type="name" class="form-control" id="exampleInputEmail1" name="name" aria-describedby="emailHelp" value="<?php echo $_SESSION['person_name'] ?>" />
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <input
            ...
            name="email"
            value="<?php echo $_SESSION['person_email']?>"
          />
